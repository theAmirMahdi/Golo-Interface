...
function countTaskBoxes(columnClass) {
  const column = document.querySelector(`.${columnClass}`);
  const taskBoxes = column.querySelectorAll(".task-box");
  return taskBoxes.length;
}
...
function updateCounters() {
  ...
  columnClasses.forEach((columnClass) => {
    const count = countTaskBoxes(columnClass);
    const column = document.querySelector(`.${columnClass}`);
    const counterElement = column.querySelector(
      ".column-header-details .counter"
    );
    if (counterElement) {
